> **이번 회차(2회차) 실습은 [docs/PART3.md](docs/PART3.md) 입니다 — 기존 Skill(K-Dense scanpy)과 MCP(BioMCP · OLS)를 내 워크플로우에 맞게 **변형**하는 30분.** 아래 5단계는 이전 회차(연결·게이트·자산화) 자료입니다.
...
> 이번엔 Claude 가 **직접 서식을 짓지 않고** `report.py` 로 `report.html` 을 만들고 `check_sop.py` 로 검증합니다.
> (SKILL.md 에 그렇게 하라고 적혀 있으니까 — **설명이 곧 인터페이스**, 2단계 그 원리)
> 결과 `report.html` 을 브라우저로 열어보면 **회사 표준 참고자료처럼** 렌더됩니다(라이트/다크 대응).

### 4c. 자산화의 증거 — 눈으로

```bash
cd .claude/skills/lab-report
python3 scripts/report.py run_data.sample.json > /tmp/r1.html
python3 scripts/report.py run_data.sample.json > /tmp/r2.html
diff /tmp/r1.html /tmp/r2.html                    # 빈 결과 = 완전히 동일
python3 scripts/check_sop.py /tmp/r1.html         # PASS — 6요소 충족
python3 scripts/check_sop.py prose_sample.html    # FAIL — CL ID·재현정보 누락
```

> 말로 쓴 버전(4a)은 흔들리고, 자산(4b)은 **몇 번을 돌려도 동일**(빈 diff) + **check 가 누락을 판정**.
> `prose_sample.html` 는 겉보기엔 멀쩡한데 Cell Ontology ID(**3단계 OLS 값!**)·재현 정보가 빠졌다고 콕 집힙니다.
>
> **열어서 비교** — `/tmp/r1.html`(표준)과 `prose_sample.html`(즉흥)을 브라우저(또는 VS Code Live Preview)로 나란히 열면, **얼마나 정형화됐는지** 눈에 확 들어옵니다.
